...
function checksExistsUserAccount(request, response, next) {
  //  Esse middleware é responsável por receber o username do usuário pelo header e validar 
  //  se existe ou não um usuário com o username passado. 
...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  // no **plano grátis e ainda não possuir 10 *todos* cadastrados** 
  ...
  if (((user.pro === false) && (user.todos.length < 10)) || user.pro === true) {
    ...
    return response.status(403).json({ error: 'Todo bigger then 10 or plan free' });
  }
}
...
function findUserById(request, response, next) {
  ...
  // Caso o usuário tenha sido encontrado, o mesmo deve ser repassado para dentro do request.
  // user e a função next deve ser chamada.
  const { id } = request.params;
